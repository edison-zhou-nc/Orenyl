...
def test_decode_salt_logs_warning_when_insecure_dev_salt_used(caplog):
    """When ORENYL_ALLOW_INSECURE_DEV_SALT=1 and no salt is configured, a warning is logged."""
    env = {
        "ORENYL_ALLOW_INSECURE_DEV_SALT": "1",
    }
    with mock.patch.dict(os.environ, env, clear=False):
        os.environ.pop("ORENYL_ENCRYPTION_SALT", None)
        with caplog.at_level(logging.WARNING, logger="orenyl.encryption"):
            result = _decode_salt("ORENYL_ENCRYPTION_SALT")
...
def test_decode_salt_no_warning_when_real_salt_configured(caplog):
    ...
    env = {"ORENYL_ENCRYPTION_SALT": real_salt}
...
def test_decode_salt_uses_process_local_fallback_not_static_literal(caplog):
    ...
    with mock.patch.dict(os.environ, env, clear=False):
        os.environ.pop("ORENYL_ENCRYPTION_SALT", None)
        with caplog.at_level(logging.WARNING, logger="orenyl.encryption"):
            first = _decode_salt("ORENYL_ENCRYPTION_SALT")
            second = _decode_salt("ORENYL_ENCRYPTION_SALT")
