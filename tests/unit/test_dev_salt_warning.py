...
from lore.encryption import _decode_salt
...
def test_decode_salt_logs_warning_when_insecure_dev_salt_used(caplog):
    ...
    with mock.patch.dict(os.environ, env, clear=False):
        ...
        with caplog.at_level(logging.WARNING, logger="lore.encryption"):
            result = _decode_salt("LORE_ENCRYPTION_SALT")

    assert len(result) >= 16
    assert result != b"lore-default-salt!"
...
def test_decode_salt_uses_process_local_fallback_not_static_literal(caplog):
    ...
    assert first == second
    assert len(first) >= 16
    assert first != b"lore-default-salt!"
