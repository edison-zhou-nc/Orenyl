...
def test_resolve_runtime_keyring_uses_active_version_and_legacy_env(monkeypatch):
    ...
    monkeypatch.setenv("LORE_ENCRYPTION_PASSPHRASE", "legacy-pass")
...
def test_encrypt_payload_carries_key_version_and_round_trips(monkeypatch):
    ...
    monkeypatch.setenv("LORE_ENCRYPTION_PASSPHRASE_V1", "pw-v1")
    monkeypatch.setenv("LORE_ENCRYPTION_SALT_V1", base64.b64encode(b"0123456789abcdef").decode("ascii"))
...
def test_resolve_runtime_keyring_fails_without_required_salt(monkeypatch):
    ...
    monkeypatch.setenv("LORE_ENCRYPTION_PASSPHRASE_V2", "pw-v2")
