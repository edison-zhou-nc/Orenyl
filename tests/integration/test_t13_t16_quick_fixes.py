...
def test_context_pack_build_domain_scopes_fact_selection():
    ...
    e1 = Event(id="event:test:h", type="med_started", payload={"name": "metformin"}, domains=["health"])
    e2 = Event(id="event:test:c", type="role_assigned", payload={"user": "u", "role": "admin"}, domains=["career"])
    db.insert_event(e1)
    db.insert_event(e2)
