...
This guide is for early-production operators who want Orenyl exposed over authenticated `streamable-http`. It is intentionally compact: enough to stand up a safe public-beta deployment without turning into a full infrastructure manual.

## When to use this mode

Use authenticated `streamable-http` when Orenyl is behind a real network boundary and tool calls must be authorized per request. Keep `stdio` for local development only.

## Required environment

At minimum, set these paths and transport settings:

- `ORENYL_TRANSPORT=streamable-http`
- `ORENYL_DB_PATH` for the primary SQLite memory store
- `ORENYL_AUDIT_DB_PATH` for audit and security events
- `ORENYL_DR_SNAPSHOT_DIR` for disaster-recovery snapshots

For authenticated deployments, also configure one JWT family:

- `ORENYL_OIDC_ALLOWED_ALGS=RS256` with issuer and JWKS settings for OIDC / RS256
- `ORENYL_OIDC_ALLOWED_ALGS=HS256` with a shared secret for controlled environments

## Example setups

Examples below use PowerShell syntax. The same environment variables apply on Linux and macOS; translate the assignments to your shell before starting Orenyl.
...
- Treat `ORENYL_DR_SNAPSHOT_DIR` as part of the recovery path, not a scratch directory.
- Back up the SQLite files and snapshot artifacts together so audit and memory state stay aligned during restore.
- If encryption is enabled, keep `ORENYL_ENCRYPTION_PASSPHRASE` and `ORENYL_ENCRYPTION_SALT` in the same secret-management flow as your JWT material.
