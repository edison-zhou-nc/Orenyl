...
def _build_wheel_smoke_script() -> str:
    return "\n".join(
        [
            ...
            "wheel = next(dist.glob('orenyl_mcp_server-*.whl'))",
            "venv_dir = pathlib.Path(tempfile.mkdtemp(prefix='orenyl-smoke-'))",
            "try:",
            ...
            "    subprocess.run([str(python_bin), '-c', 'import orenyl, orenyl.server'], check=True)",
            "finally:",
